...
const PercentageCalculator = () => {
  const [activeTab, setActiveTab] = useState<'basic' | 'increase' | 'decrease' | 'difference'>('basic');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-xl text-gray-600">Calculate percentages with ease</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                  activeTab === 'basic' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
                }`}
              >
                Basic %
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                  activeTab === 'increase' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
                }`}
              >
                % Change
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                  activeTab === 'decrease' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
                }`}
              >
                X of Y
              </button>
                ...
                {basicResult !== null && (
                  <div className="p-4 bg-blue-50 rounded-lg">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{basicResult}</div>
            ...
            {activeTab === 'increase' && (
              <div className="space-y-6">
                ...
                {changeResult !== null && (
                  <div className="p-4 bg-blue-50 rounded-lg">
                    <div className="text-center">
                      <div className={`text-2xl font-bold ${changeResult >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {changeResult >= 0 ? '+' : ''}{changeResult}%
                      </div>
                      <div className="text-sm text-gray-600">
                        {changeResult >= 0 ? 'Increase' : 'Decrease'} from {originalValue} to {newValue}
                      </div>
                    </div>
                  </div>
                )}
            ...
            {activeTab === 'decrease' && (
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-900">What percentage is X of Y?</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Part Value (X)
                ...
                {ratioResult !== null && (
                  <div className="p-4 bg-blue-50 rounded-lg">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{ratioResult}%</div>
                      <div className="text-sm text-gray-600">
